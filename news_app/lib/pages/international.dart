...
import 'package:news_app/blocs/internationa_news_bloc.dart';
...
class _InternationalNewsState extends State<InternationalNews> with AutomaticKeepAliveClientMixin {
  ...
  void initState() {
    ...
    Future.delayed(Duration(milliseconds: 0))
      ...
      context.read<InternationalNewsBloc>().getData(mounted,true);
    });
  ...
  void _scrollListener() {
    final db = context.read<InternationalNewsBloc>();

    if (!db.isLoading) {
      if (controller.position.pixels == controller.position.maxScrollExtent) {
        context.read<InternationalNewsBloc>().setLoading(true);
        context.read<InternationalNewsBloc>().getData(mounted,true);
  ...
  Widget build(BuildContext context) {
    super.build(context);
    final cb = context.watch<InternationalNewsBloc>();
      ...
      appBar: AppBar(
        ...
        actions: <Widget>[
          IconButton(
            ...
            onPressed: (){
              context.read<InternationalNewsBloc>().onRefresh(mounted,true);
            },
          )
        ],
      ),
      ...
      body: RefreshIndicator(
        child: cb.hasData == false
          ...
          children: [
            ...
            EmptyPage(icon: Feather.clipboard, message: 'no countries found'.tr(), message1: ''),
          ],
        )
